...
abstract class BaseLandmarksDetector {
    ...
    abstract detectLandmarks(image: Mat, boundingBox?: any, faceDetector?: any): any[];
}
...
export class FANLandmarksDetector extends BaseLandmarksDetector {
    ...
    detectLandmarks(image: Mat, boundingBox: any = null, faceDetector: any = null): any[] {
    ...
    private _preprocessingFAN(img: Mat, detect: boolean = false, faceDetector: any = null, bbox: any = null): [Mat, Mat, number] {
        // if (img.dims === 2) {
		...
		let newImg = new cv2.Mat();
		let center = new cv2.Mat();
    ...
    private _crop(image: Mat, center: Mat, scale: number, resolution: number = 256.0): Mat {
    ...
    private _transform(point: number[], center: Mat, scale: number, resolution: number, invert: boolean = false): Mat {
		...
		const newPoint = new cv2.Mat();

        return newPoint;
    }
	
	private _getPredsFromhm(hm: Mat, center: Mat | null = null, scale: number | null = null): [Mat[], Mat[]] {
	//     if (hm.ndim !== 4) {
	...
	drawLandmarks(image: Mat, landmarks: [number, number][], color: [number, number, number]): Mat {
	    for (let i = 0; i < landmarks.length; i++) {
	        const [x, y] = landmarks[i];
	        cv2.circle(image.clone(), new cv2.Point(Math.floor(x), Math.floor(y)), 3, new cv2.Scalar(color[0], color[1], color[2]), -1);
	    }
	    return image;
	}
}